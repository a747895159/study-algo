...
public class DGTest004 {
    ...
    public static int dp_OPT(int n, int bag) {
        ...
        for (int i = 1; i < weight.length; i++) {
             ...
             * 倒序可以避免重复计算，避免重复使用，确保在更新DP数组中的某个容量值时，不会受到后面更大容量值的影响，从而避免了重复计算的问题。
             * 必须倒序遍历背包容量（即从大容量到小容量）。for (int j = bag; j >= weight[i]; j--) {
             * 如果是完全背包，则正序遍历背包容量即可。 for (int j = weight[i]; j <= bag; j++){
             */
